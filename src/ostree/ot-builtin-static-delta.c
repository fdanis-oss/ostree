...
ot_static_delta_builtin_verify (int argc, char **argv, OstreeCommandInvocation *invocation, GCancellable *cancellable, GError **error)
{
  ...
  if (!ostree_repo_static_delta_is_signed (repo, delta_id, &array, error))
    {
      if (*error)
        {
          g_debug ("Unable to find static-delta signature(s): %s", (*error)->message);
          g_clear_error (error);
        }
      g_print ("Not signed\n");
      return TRUE;
  ...
  gchar **signatures_v = (gchar **)g_ptr_array_free(array, FALSE);
  g_autofree char *signatures = g_strjoinv(", ", signatures_v);
  g_strfreev(signatures_v);
  g_print ("Signed with: %s\n", signatures);
  ...
  verified = ostree_repo_static_delta_verify_signature (repo, delta_id, sign, error);
